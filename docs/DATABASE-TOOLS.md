...
IntelliJ-based IDEs from 2025.1) provides comprehensive database tooling natively. Use those
tools. AgentBridge only supplements with the one thing the native MCP does not cover.

---

## JetBrains' native database MCP tools

Available when both the **Database Tools and SQL** plugin and the **JetBrains AI Assistant**
plugin are installed. See https://www.jetbrains.com/help/idea/mcp-server.html#database_specific_tools
...
These are official, tested, maintained JetBrains tools. Use them instead of building duplicates.

---

## What AgentBridge provides

AgentBridge provides exactly one database tool: **`database_add_source`** (experimental plugin
only). This fills the one gap in JetBrains' native MCP — adding a new data source programmatically
