...
// Generate plugin.xml by merging plugin-core's descriptor with custom-mcp extensions
val generatePluginXml by tasks.registering {
    ...
    val customMcpExtras = file("src/main/resources/META-INF/custom-mcp-extensions.xml")
    val outputDir = layout.buildDirectory.dir("generated/plugin-xml/META-INF")

    inputs.file(corePluginXml)
    inputs.file(customMcpExtras)
    outputs.dir(outputDir)

    doLast {
        var xml = corePluginXml.readText()
        val extras = customMcpExtras.readText()

        // Insert custom-mcp extensions before closing </extensions>
        xml = xml.replace("  </extensions>", "$extras\n  </extensions>")

        // Distinguish the plugin name in the IDE plugins list
        xml = xml.replace(
            "<name>IDE Agent for Copilot</name>",
            "<name>IDE Agent for Copilot (Custom MCP)</name>"
        )
...
tasks.named<ProcessResources>("processResources") {
    ...
    // Exclude the template fragment — only the generated plugin.xml goes into the JAR
    exclude("META-INF/custom-mcp-extensions.xml")
}
...
tasks.named<Zip>("buildPlugin") {
    archiveBaseName.set("ide-agent-for-copilot-custom-mcp")
...
intellijPlatform {
    pluginConfiguration {
        ...
        name = "IDE Agent for Copilot (Custom MCP)"
