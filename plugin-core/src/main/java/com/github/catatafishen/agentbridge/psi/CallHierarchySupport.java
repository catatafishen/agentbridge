...
public class CallHierarchySupport {
    ...
    private static PsiNameIdentifierOwner resolveNamedElementAtLocation(@NotNull Project project,
        ...
        // Shared declaration-first / reference-fallback resolution. The reference fallback
        // handles the case where the caller pointed at a usage (e.g. a call site for printf
        // inside main()) rather than a declaration — works in any IDE whose PSI registers
        // reference providers (Java, Kotlin, C/C++ in CLion, Python in PyCharm, Go in GoLand,
        // JS/TS in WebStorm, etc.).
        return ToolUtils.resolveNamedElement(ctx, elementName);
    }
